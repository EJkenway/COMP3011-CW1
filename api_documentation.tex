...
This document provides comprehensive documentation for the Smart Productivity Analytics Platform API. The API enables task management, habit tracking, Pomodoro timer functionality, and productivity analytics through a RESTful interface.
...
\bottomrule
\end{tabular}
\caption{Habit Endpoints}
\end{table}

\subsubsection{Habit Object}
...
\subsubsection{Complete Habit}

\textbf{POST} \texttt{/api/v1/habits/\{id\}/complete/}

\textbf{Request Body:}
\begin{lstlisting}[style=json]
{
    "notes": "Completed 15 minutes today"
}
\end{lstlisting}
...
\begin{lstlisting}[style=json]
{
    ...
    "message": "Habit completed! Current streak: 16"
}
\end{lstlisting}

% -----------------------------------------------------------------------------
\subsection{Pomodoro Endpoints}
% -----------------------------------------------------------------------------
...
POST & /pomodoro/ & Start a new session \\
GET & /pomodoro/\{id\}/ & Get session details \\
DELETE & /pomodoro/\{id\}/ & Delete session \\
POST & /pomodoro/\{id\}/complete/ & Complete session \\
POST & /pomodoro/\{id\}/cancel/ & Cancel session \\
GET & /pomodoro/active/ & Get active session \\
GET & /pomodoro/today\_summary/ & Get today's summary \\
\bottomrule
\end{tabular}
\caption{Pomodoro Endpoints}
\end{table}

\subsubsection{Start Pomodoro Session}

\textbf{POST} \texttt{/api/v1/pomodoro/}
...
Session types: \texttt{focus}, \texttt{short\_break}, \texttt{long\_break}
...
GET & /analytics/pomodoro/ & Pomodoro analytics \\
...
start\_pomodoro & Start a focus session \\
complete\_habit & Log habit completion \\
...
weekly & Complete every week \\
monthly & Complete every month \\
\bottomrule
\end{tabular}
\caption{Habit Frequency Values}
\end{table}
...
# Log completion
curl -X POST http://localhost:8000/api/v1/habits/<id>/complete/ \
  -H "Authorization: Bearer <token>"
\end{lstlisting}

\subsubsection{4. Use Pomodoro Timer}
...
curl -X POST http://localhost:8000/api/v1/pomodoro/ \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"session_type":"focus","planned_duration":25}'

# Complete session
curl -X POST http://localhost:8000/api/v1/pomodoro/<id>/complete/ \
  -H "Authorization: Bearer <token>" \
  -d '{"interruptions_count":1}'
\end{lstlisting}
